	...
	newPath(pt){
		...
		if (this.calculatedPath && this.calculatedPath.length) {
			...
			if (this.app.debug.showPath && !this.npc){
                ...
                const positionsArray = new Float32Array(this.calculatedPath.length*3+3);
                positionsArray[0] = player.position.x;
                positionsArray[1] = player.position.y;
                positionsArray[2] = player.position.z;

                // Draw debug lines
                const index = 0;
				this.calculatedPath.forEach( function(vertex){
                    let v = vertex.clone().add(new THREE.Vector3(0, self.app.debug.offset, 0))
                    index = index+1;
                    positionsArray[index*3] = v.x;
                    positionsArray[index*3+1] = v.y;
                    positionsArray[index*3+2] = v.z;
				});
